...
*/
	...
	if(!$(".wizard-card form").valid()){
        ...
        //if success then send data to page
        var form = $('#formWizard');
        var data = new FormData();
        data.append('firstName',form.find('input[name="firstname"]').val());
        data.append('lastName',form.find('input[name="lastname"]').val());
        data.append('email',form.find('input[name="email"]').val());
        data.append('country',form.find('#selectCountry').val());
        $.each($('#wizard-picture')[0].files,function(key,value){
            data.append(key,value);
        });
        console.log($('#wizard-picture')[0].files[0]);
        $.ajax({
            url: base+"user/profile_update/wizard",
